...
def _expected_from_spec(spec_path: Path) -> int:
    ...
    # contar entradas tal cual vienen en "moves" (incluye sub-movimientos)
    mv = d.get("moves", [])
    return int(len(mv))
...
def main():
    ...
    ap.add_argument("--vstart", type=float, default=0.60, help="Umbral arranque velocidad (u/s)")
    ap.add_argument("--vstop", type=float, default=0.20, help="Umbral parada velocidad (u/s)")
    ap.add_argument("--min-dur", type=float, default=0.25, help="Duración mínima segmento (s)")
    ap.add_argument("--min-gap", type=float, default=0.18, help="Fusión de huecos < (s)")
    ap.add_argument("--poly-n", type=int, default=20, help="Puntos polilínea trayectoria")
    ap.add_argument("--min-path-norm", type=float, default=0.02, help="Longitud mínima de trayectoria (0..1) para aceptar el segmento")
